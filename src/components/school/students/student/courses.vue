<template>
  <div dir="auto" class="bg-White rounded-v flex-1 flex-col gap-4 p-4">
    ...
    <div v-if="!data.student.isNew && courses.length" class="h-full space-y-4" :class="{ 'hidden sm:block': !compressed }">
    ...
    <form @submit.prevent="submitForm" v-if="store.getters.permission('students:payments:add') && !data.student.isNew && !getting && selected.course"
