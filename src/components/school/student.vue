<template>
  ...
  <div class="w-full h-fit sm:h-auto sm:flex sm:flex-col space-y-4 overflow-y-auto">
    <div v-if="[false, 'informations', 'courses'].includes(zoom)" class="w-full sm:flex sm:gap-4 space-y-4 sm:space-y-0" :class="{ 'h-full': zoom, 'sm:h-1/2': !zoom}">
      <student-informations v-if="[false, 'informations'].includes(zoom)" @zoom="zoom = (zoom != 'informations' && 'informations')" :data="{ zoom, school, student }" />
      <student-courses v-if="[false, 'courses'].includes(zoom)" :zoom="zoom" @zoom="zoom = (zoom != 'courses' && 'courses')" :data="{ zoom, school, student }" :class="{ 'hidden sm:flex': student.isNew, 'flex': !student.isNew }" />
    </div>
    <div v-if="[false, 'payment', 'presence'].includes(zoom)" class="w-full sm:flex sm:gap-4 space-y-4 sm:space-y-0" :class="{ 'h-full': zoom, 'sm:h-1/2': !zoom, 'hidden': student.isNew }">
      <student-payments v-if="[false, 'payment'].includes(zoom)" :zoom="zoom" @zoom="zoom = (zoom != 'payment' && 'payment')" :data="{ zoom, school, student }" />
      <student-presence v-if="[false, 'presence'].includes(zoom)" :zoom="zoom" @zoom="zoom = (zoom != 'presence' && 'presence')" :data="{ zoom, school, student }" />
    </div>
  </div>
</template>
